...
void cons_runcmd(char *cmdline, CONSOLE *cons, int *fat, unsigned int memtotal){
	...
	if(strcmp(cmd, "mem") == 0){
		...
		cmd_cat(cons, para, fat);
	}else if(strcmp(cmd, "hlt") == 0){
		cmd_hlt(cons, fat);
	}else if(cmd[0] != 0){
		putStrOnSheet(cons->sheet, cons->cur_x, cons->cur_y, COL8_FFFFFF, "Bad commend!");
		cons_newline(cons);
		cons_newline(cons);
	}
	return;
...
void cmd_mem(CONSOLE *cons, unsigned int memtotal){
	...
	putStrOnSheet(cons->sheet, cons->cur_x, cons->cur_y, COL8_FFFFFF, s);
	cons_newline(cons);
	...
	for(x = 0; x < 224; x++){
		...
		if(finfo[x].name[0] != 0xe5){
			if((finfo[x].type & 0x18) == 0){
				...
				s[10] = finfo[x].ext[1];
				s[11] = finfo[x].ext[2];
				putStrOnSheet(cons->sheet, 8, cons->cur_y, COL8_FFFFFF, s);
				cons_newline(cons);
			}
		}
	}
...
void cmd_cat(CONSOLE *cons, char *fname, int *fat){
	...
	if(finfo != NULL){	/* 找到文件的情况 */
		...
		putStrOnSheet(cons->sheet, 8, cons->cur_y, COL8_FFFFFF, "File Not Found!");
		cons_newline(cons);
	}
	cons_newline(cons);
	return;
